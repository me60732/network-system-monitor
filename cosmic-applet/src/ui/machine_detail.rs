...
use cosmic::widget::{button, column, container, row, scrollable, text, canvas};
...
pub fn view(
    ...
    let machine_name = machine_config.name.clone();
    ...
    let gpu_chart_visible = minimon_config.gpus.get("default")
    ...
    // Machine name as title
    let machine_title = text(machine_name)
        .size(18)
        .into();
    
    // Show the compact panel widget (shows what's currently visible with charts)
    let panel_widget = PanelWidget::view_single_machine(remote_machine, &content_order, minimon_config);
    
    // Build detail sections for enabled sensors NOT visible in panel
    let mut metrics_items: Vec<Element<'static, AppMessage>> = vec![
        back_button,
        machine_title,
        panel_widget,
    ];
        ...
        metrics_items.push(
            container(
                column(vec![
                    ...
                    row(vec![
                        canvas(ring).width(48).height(48).into(),
                        text(format!("{:.2} GB ({:.1}%)", memory_gb, memory_percent)).size(14).into(),
                    ])
                    .spacing(12)
                    .into(),
                ])
                ...
                disk_items.push(
                    column(vec![
                        row(vec![
                            ...
                            text(format!("{:.1}GB / {:.1}GB ({:.1}%)", used_gb, total_gb, used_percent)).size(12).into(),
                        ])
                        .spacing(8)
                        .into(),
                        container(
        ...
        metrics_items.push(
            container(
                column(vec![
                    ...
                    row(vec![
                        canvas(ring).width(48).height(48).into(),
                        text(format!("{:.2} GB ({:.1}%)", gpu_gb, gpu_percent)).size(14).into(),
                    ])
                    .spacing(12)
                    .into(),
                ])
    ...
    let content = column(metrics_items)
        .spacing(16)
        .padding([20, 16]);
